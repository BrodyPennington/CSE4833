...
int main() {
    ...
    for (int size = 10; size <= 10000; size += 10) {
        ...
        // selectionSort time in miliseconds
        double selectTime = timer.stop();


        timer.start();
        mergeSort(arrMergeSort, 0, arrMergeSort.size() - 1);
        // mergeSort time in miliseconds
        double mergeTime = timer.stop();


        outFile << size << "," << selectTime << "," << mergeTime << "\n";


        std::cout << size << ", " << selectTime << ", " << mergeTime << std::endl;

        if (!foundCrossover && mergeTime < selectTime) {

            foundCrossover = true;
            crossoverSize = size;

            std::cout << ">>> Merge sort is faster at size " 
                      << size << "!\n";
